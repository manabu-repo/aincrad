...
- 合成事件：`React` 通过合成事件来处理浏览器原生事件，提供了一种跨浏览器的事件处理机制，可以处理事件冒泡、事件委托等。
  - 好处是创建了一个跨浏览器的事件对象，屏蔽了不同浏览器在事件处理上的差异，让开发者无需担心兼容性问题，无论在哪个浏览器环境下，事件的使用方式都是统一的。
- 事件委托：`React` 使用事件委托来处理事件，将事件绑定到根元素上，通过事件冒泡机制来处理事件。
  - 好处是减少事件绑定的次数，提高性能，减少内存消耗，提高事件处理效率。
- 事件传播：事件传播分为捕获阶段、目标阶段和冒泡阶段，`React` 会在捕获阶段和冒泡阶段执行事件处理函数。
  - 捕获阶段：事件从根元素向目标元素(`逐级向下`)传播。
  - 冒泡阶段：事件从目标元素向根元素(`逐级向上`)传播。
  - 阻止事件传播：可以通过 `event.stopPropagation()` 方法来阻止事件传播，阻止事件继续向上或向下传播。
- 事件绑定：在 `React` 中，事件是通过 `onEventName` 的方式绑定的，例如 `onClick`、`onChange`、`onSubmit` 等。在组件挂载时，`React` 会将事件绑定到对应的元素上，在组件卸载时，会解绑事件。
